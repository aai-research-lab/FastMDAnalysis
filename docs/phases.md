...
`qvalue`, `dihedrals`, `cluster`, and `dimred`. The compatibility profile
`--analyze-compatibility-profile fastmdanalysis` preserves the historical
FastMDAnalysis defaults where that comparison is required.
