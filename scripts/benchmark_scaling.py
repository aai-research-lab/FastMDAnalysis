...
Optionally, you can keep full-workflow artifacts (figures, *.dat files) for
selected frame counts using --keep-artifacts.
"""
...
def write_json(records: Sequence[Dict[str, object]], path: Path) -> None:
    ...
    payload = {
        "metadata": {
            ...
            "dataset": "Ubiquitin",
            "notes": "Full workflow benchmark scaling study.",
        },
...
def persist_artifacts_for_frames(
    frames_to_keep: Sequence[int],
    tools: Sequence[str],
    output_dir: Path,
) -> None:
    """Re-run full workflows for selected frame counts and KEEP artifacts.

    For each tool and each requested frame count, this will:
      - temporarily set base_benchmark.FRAME_SLICE = (0, frames, 1)
      - call the corresponding benchmark_*_single function
      - write all artifacts into:

            output_dir / "<tool>_artifacts" / f"frames_{frames}"

        i.e.
            fastmdanalysis -> benchmark_output/fastmdanalysis_artifacts/frames_500/
            mdtraj         -> benchmark_output/mdtraj_artifacts/frames_500/
            mdanalysis     -> benchmark_output/mdanalysis_artifacts/frames_500/
    """
    if not frames_to_keep:
        return

    print("\nPersisting full-workflow artifacts for selected frame counts:")
    for frames in frames_to_keep:
        ...
        for tool in tools:
            ...
            tool_dir_name = {
                "fastmdanalysis": "fastmdanalysis_artifacts",
                "mdtraj": "mdtraj_artifacts",
                "mdanalysis": "mdanalysis_artifacts",
            }[tool]

            outdir = output_dir / tool_dir_name / f"frames_{frames}"
            outdir.mkdir(parents=True, exist_ok=True)

            print(f"    [{display_name}] saving artifacts to {outdir} ...")

            previous_slice = base_benchmark.FRAME_SLICE
            base_benchmark.FRAME_SLICE = (0, frames, 1)
            try:
                if tool == "fastmdanalysis":
                    runtime, memory = base_benchmark.benchmark_fastmda_single(str(outdir))
                elif tool == "mdtraj":
                    runtime, memory = base_benchmark.benchmark_mdtraj_single(str(outdir))
                elif tool == "mdanalysis":
                    runtime, memory = base_benchmark.benchmark_mdanalysis_single(str(outdir))
                else:
                    print(f"      ! Unknown tool key {tool}, skipping.")
                    continue

                if runtime is None or memory is None:
                    print(f"      ! {display_name}: no metrics returned (runtime/memory None).")
                else:
                    print(
                        f"      ✓ {display_name} artifacts saved "
                        f"(runtime={runtime:.3f}s, peak_mem={memory:.1f} MB)"
                    )
            finally:
                base_benchmark.FRAME_SLICE = previous_slice
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory to store metrics and (optionally) artifacts (default: benchmark_output).",
    )
    parser.add_argument(
        "--json-name",
    ...
    parser.add_argument(
        ...
        dest="warmup",
        action="store_true",
        help="Explicitly enable warm-up runs (default: disabled).",
    )
    parser.add_argument(
        "--no-warmup",
        dest="warmup",
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Re-run full workflows and keep artifacts for selected frame counts.\n"
            "Usage:\n"
            "  --keep-artifacts              (keep artifacts for the smallest frame count)\n"
            "  --keep-artifacts 500 2000     (keep artifacts for frames=500 and 2000)\n"
            "If this option is omitted entirely, no artifacts are saved."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    # Optional: keep full-workflow artifacts
    #
    # Rules:
    #   - if args.keep_artifacts is None:
    #         do not save any artifacts
    #   - if args.keep_artifacts == [] (flag provided, no values):
    #         save artifacts for the smallest frame count that was benchmarked
    #   - if args.keep_artifacts contains values:
    #         save artifacts for those frame counts that were benchmarked
    if args.keep_artifacts is not None:
        if args.keep_artifacts:
            requested = _ordered_positive_counts(args.keep_artifacts)
            frames_to_keep = [f for f in requested if f in frame_counts]
            missing = [f for f in requested if f not in frame_counts]
            for f in missing:
                print(
                    f"[warn] --keep-artifacts requested frames={f}, "
                    f"but this frame count was not benchmarked; skipping."
                )
            if not frames_to_keep:
                frames_to_keep = [min(frame_counts)]
        else:
            # Flag present but no values: keep smallest benchmarked frame count
            frames_to_keep = [min(frame_counts)]

        persist_artifacts_for_frames(frames_to_keep, tools, output_dir)
